...
async def play_tg_file(c: Client, m: Message, replied: Message = None, link: str = None):
    ...
    if replied.video or replied.document:
        ...
        if chat_id in QUEUE:
         ...
         try:
            await loser.edit("🔄 Grup Görüşmesine Katılıyorum...")
            gcname = m.chat.title
            ctitle = await CHAT_TITLE(gcname)
            title = songname
            userid = m.from_user.id
        ...
        await m.reply(
            "» reply to an **video file** or **aramak için bir şey ver.**"
        )
...
Bu botu kullanmak için gerçek kullanıcı hesabınıza geri dönün."
    ...
    try:
        ...
        if b.status == "banned":
            try:
                await m.reply_text("❌ Bu sohbette kullanıcı robotu yasaklanmış, müzik çalabilmek için önce kullanıcı robotunun yasağını kaldırın !")
                await remove_active_chat(chat_id)
            except BaseException:
                pass
        ...
        try:
            ...
            traceback.print_exc()
            return await m.reply_text(
                f"❌ **Asistan sesli sohbete katılmadı**\n\n**sebep**: `{e}`"
            )
    if replied:
        if replied.video or replied.document:
            ...
            if len(m.command) < 2:
                ...
                loser = await c.send_message(chat_id, "🔍 **Yükleniyor...**")
                ...
                if search == 0:
                    ...
                    if veez == 0:
                        ...
                        if chat_id in QUEUE:
                            await loser.edit("🔄Sıra İzleniyor...")
                            pos = add_to_queue(
                            ...
                            try:
                                ...
                                await loser.delete()
                                await remove_active_chat(chat_id)
                                await m.reply_text("❌ bot Grup aramasını bulamıyor veya etkin değil.\n\n» Use Grup çağrısını açmak için /startvc komutu !")
                            except NoVideoSourceFound:
                                await loser.delete()
                                await remove_active_chat(chat_id)
                                await m.reply_text("❌ Oynatmak için sağladığınız içeriğin video kaynağı yok")
        ...
        if len(m.command) < 2:
            await m.reply(
                "» reply to an **video dosyası** or **aramak için bir şey ver.**"
            )
...
async def vstream(c: Client, m: Message):
    ...
    try:
        ...
        b = await c.get_chat_member(chat_id, ubot)
        if b.status == "banned":
            await m.reply_text("❌ Asistan bu sohbette yasaklandı, müzik çalabilmek için önce Asistan'ın banını kaldırın!")
            return
        invitelink = (await c.get_chat(chat_id)).invite_link
        if not invitelink:
            await c.export_chat_invite_link(chat_id)
            ...
            if chat_id in QUEUE:
                ...
                try:
                    ...
                    await m.reply_photo(
                        photo=f"{IMG_2}",
                        reply_markup=InlineKeyboardMarkup(buttons),
                        caption=f"🗂 **İsim:** [{songname}]({url}) | `live`\n🧸 **Talep Eden:** {requester}",
                    )
                except (NoActiveGroupCall, GroupCallNotFound):
                    await loser.delete()
                    await remove_active_chat(chat_id)
